# -*- coding: utf-8 -*-
"""
Created on 2024-01-24 (Wed) 19:28:18
...
class HTCL(nn.Module):
    ...
    def forward(self,
        ...
        # Processing of feat_orig FIXME: Our own processing
        SMILES_TO_GRAPH = {}
        mol_graphs = []
        for smiles in smiles_batch:
            if smiles in SMILES_TO_GRAPH:
                ...
                mol_graph = featurization.MolGraph(smiles, self.args)
        ...
        for m in mol_graphs:
            f_atoms = [[0] * 133]
            f_atoms.extend(m.f_atoms)
            f_atoms = torch.FloatTensor(f_atoms) # (n_atoms, 133)
            f_atoms_sum = torch.sum(f_atoms,dim=0) # (1, 133)
            feat_orig.append(f_atoms_sum)
        feat_orig = torch.stack(feat_orig) # (544, 133)
        feat_orig = feat_orig.cuda()
        ...
        return outputs_g1, outputs_g2, DGI_loss1, DGI_loss2